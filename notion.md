# OWASP Web App Security Testing Checklist

> **How to use:** Import this file into Notion via *Settings → Import → Text & Markdown*. Checkboxes will become interactive to-do blocks. Duplicate this page for each engagement.
...
# 02 — Configuration & Deployment Testing
...
- [ ] Check how the app handles `4xx` and `5xx` errors
- [ ] Check the privilege required to run
...
- [ ] Ensure proper naming of old and new backup files
...
- [ ] Ensure the same user or identity can't register repeatedly
...
- [ ] Check verification for the provisioning process
- [ ] Check verification for the de-provisioning process
...
- [ ] Check response with valid username + valid password
- [ ] Check response with valid username + invalid password
- [ ] Check response with invalid username + invalid password
...
- [ ] Ensure account locks after 3–5 incorrect attempts
...
- [ ] Test access using captured session ID
- [ ] Test whether multiple simultaneous logins are allowed
...
- [ ] Check that users can't use their username as a password
- [ ] Check for usage of common weak passwords
- [ ] Check the minimum password length requirement
- [ ] Check the maximum password length requirement
...
## Test For Weak Auth In Alternative Channels
...
- [ ] Ensure no cookie operations take place over unencrypted channel
...
- [ ] Test by replacing `<` and `>` with HTML entities
...
## Test For Stored XSS
- [ ] Identify stored input parameters reflected on client-side
...
- [ ] Test SQLi on authentication forms
- [ ] Test SQLi on the search bar
- [ ] Test SQLi on editable characteristics
- [ ] Find SQL keywords or entry point detections
...
- [ ] Try SQLi with null bytes before the query
- [ ] Try SQLi with URL encoding
- [ ] Try SQLi with both lower and upper cases
- [ ] Try SQLi with SQL Tamper scripts
- [ ] Try SQLi with time delay payloads
- [ ] Try SQLi with conditional delays
...
- [ ] Search for SSRF keywords in request header and body
- [ ] Identify the injection points
...
- [ ] Test error handling by triggering all possible errors

---

# 09 — Weak Cryptography Testing
...
- [ ] Test by inserting payload inside an image using `bmp.pl`
...
- [ ] Try to bypass 2FA by changing email or password
...
## JWT Testing
- [ ] Test with the `none` algorithm to bypass signature verification
...
- [ ] Check if the JWT `kid` header is injectable (SQLi or path traversal)
- [ ] Check if sensitive data (PII, passwords) is stored in JWT payload
...
- [ ] Test for GraphQL batching attacks (multiple queries in one request)
- [ ] Test for deeply nested queries causing DoS (no query depth limit)
- [ ] Test for field duplication / alias-based query amplification
...
- [ ] Test for blind XXE via out-of-band DNS/HTTP callbacks (Burp Collaborator)
...
- [ ] Test for RCE via deserialization gadget chains
...
- [ ] Test for CL.TE smuggling (front-end: Content-Length, back-end: Transfer-Encoding)
- [ ] Test for TE.CL smuggling (front-end: Transfer-Encoding, back-end: Content-Length)
- [ ] Test for TE.TE smuggling (obfuscated Transfer-Encoding header)
...
- [ ] Test for bypassing front-end security controls via smuggling

## Web Cache Poisoning / Cache Deception
- [ ] Identify unkeyed inputs (headers, cookies, params) that affect the response
- [ ] Test for poisoning via `X-Forwarded-Host`, `X-Forwarded-For`, `X-Original-URL`
- [ ] Test for poisoning the cache with XSS payloads
- [ ] Test for fat GET requests (body params in cacheable GET requests)
- [ ] Test for cache deception — caching sensitive user-specific responses
...
- [ ] Use `ppmap` or Burp's DOM Invader for automated detection
- [ ] Test via `JSON.merge`, `lodash.merge`, or `jQuery.extend`
...
- [ ] Test for Cross-Site WebSocket Hijacking (CSWSH) — missing Origin check
- [ ] Test for authorization bypass on WebSocket channels
- [ ] Test for insecure data exposure over WebSocket messages
- [ ] Ensure WebSocket connections use `wss://` (TLS), not plain `ws://`
- [ ] Test for DoS via large or malformed WebSocket frames
...
- [ ] Identify `unsafe-inline` directives — allows inline script execution
- [ ] Identify `unsafe-eval` directives — allows `eval()` usage
- [ ] Check for overly permissive `script-src *` or `default-src *`
- [ ] Test for CSP bypass via JSONP endpoints on whitelisted domains
- [ ] Test for CSP bypass via AngularJS CSTI on whitelisted CDNs
- [ ] Test for CSP bypass via open redirects on whitelisted domains
- [ ] Check if `report-uri` or `report-to` is configured
- [ ] Test for `data:` URI bypass in `img-src`

## HTTP Security Headers Audit
- [ ] Ensure `Strict-Transport-Security` is set with `includeSubDomains` and `preload`
- [ ] Ensure `X-Content-Type-Options: nosniff` is set
- [ ] Ensure `X-Frame-Options: DENY` or `SAMEORIGIN` is set
- [ ] Ensure `Referrer-Policy` is set to `no-referrer` or `strict-origin-when-cross-origin`
- [ ] Ensure `Permissions-Policy` disables unused browser features
- [ ] Ensure `Cache-Control: no-store` is set on sensitive pages
- [ ] Ensure `X-Powered-By` and `Server` headers are removed or obscured
- [ ] Check for information leakage in custom headers

## Subresource Integrity (SRI)
- [ ] Ensure all externally hosted scripts have `integrity` attributes
...
- [ ] Test for `redirect_uri` bypass using path traversal
- [ ] Test for `redirect_uri` bypass using subdomains
...
- [ ] Test if the build system prefers public over internal registry for same-named packages
...
- [ ] Check for exposed CI/CD config files (`.github/workflows`, `Jenkinsfile`)
...
- [ ] Check for exposed debug endpoints (`/debug`, `/actuator`, `/metrics`)
...
- [ ] Check for DMARC record (`_dmarc.domain.com`) — ensure policy is `reject` or `quarantine`
...
- [ ] Test for open mail relay — server used to send arbitrary emails
...
*Authorized testing only. Always obtain written permission before testing any system.*

---

Maintained by **Sankar** — [GitHub](https://github.com/sankarlmao/) · [LinkedIn](https://www.linkedin.com/in/sankarcy/)